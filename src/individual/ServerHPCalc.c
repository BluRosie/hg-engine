        ...
        if ((sp->battlemon[sp->defence_client].condition2 & STATUS2_SUBSTITUTE)
            ...
            if (scenario != NULL && TestBattle_HasMoreExpectations()) {
                ...
                if (activeExpectationIndex < MAX_EXPECTATIONS
                    && scenario->expectations[activeExpectationIndex].expectationType == EXPECTATION_TYPE_HP_BAR
                    && sp->defence_client == scenario->expectations[activeExpectationIndex].battlerIDOrPartySlot) {
                    for (int i = 0; i < 16; i++) {
                        ...
                        if ((u32)sp->damage == scenario->expectations[activeExpectationIndex].expectationValue.hpRecovered[i]
                            || (u32)(sp->damage * -1) == scenario->expectations[activeExpectationIndex].expectationValue.hpTaken[i]) {
                            debug_printf(" ✅");
