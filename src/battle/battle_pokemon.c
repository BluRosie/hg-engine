...
void BattleEndRevertFormChange(struct BattleSystem *bw)
{
    ...
    while (currentScenario != NULL && TestBattle_HasMoreExpectations()) {
        ...
        if (activeExpectationIndex >= MAX_EXPECTATIONS
            || currentScenario->expectations[activeExpectationIndex].expectationType == 0) {
            break;
        }
        ...
        if (currentScenario->expectations[activeExpectationIndex].expectationType == EXPECTATION_OVERWORLD_FORM) {
            // debug_printf("Checking form\n");
            struct Party *party = SaveData_GetPlayerPartyPtr(SaveBlock2_get());
            struct PartyPokemon partyPokemon = party->members[currentScenario->expectations[activeExpectationIndex].battlerIDOrPartySlot];
            u32 expectedForm = currentScenario->expectations[activeExpectationIndex].expectationValue.formID;
