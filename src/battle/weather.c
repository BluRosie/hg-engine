    ...
    do
    {
            ...
            while (sp->fcc_work < 2)
            {
                ...
                if (sp->side_condition[side] & SIDE_STATUS_REFLECT)
                {
                    if (--sp->scw[side].reflectCount == 0)
                    {
                        ...
                        LoadBattleSubSeqScript(sp, ARC_BATTLE_SUB_SEQ, SUB_SEQ_MOVE_EFFECT_FADED);
            ...
            while (sp->fcc_work < 2)
            {
                ...
                if (sp->side_condition[side] & SIDE_STATUS_SAFEGUARD)
                {
                    if (--sp->scw[side].safeguardCount == 0)
                    {
                        ...
                        LoadBattleSubSeqScript(sp, ARC_BATTLE_SUB_SEQ, SUB_SEQ_SAFEGUARD_FADED);
                        sp->next_server_seq_no = sp->server_seq_no;
                        sp->server_seq_no = 22;
                        sp->client_work = ST_ServerDir2ClientNoGet(bw, sp, side);
                        ret = 1;
        ...
        case FCC_TAILWIND:
            while (sp->fcc_work < 2)
            {
                ...
                if (sp->tailwindCount[side]) // update tailwind to use a separate counter so it can be larger
                {
                    ...
                    {
                        LoadBattleSubSeqScript(sp, ARC_BATTLE_SUB_SEQ, SUB_SEQ_TAILWIND_PETERS_OUT);
                        sp->next_server_seq_no = sp->server_seq_no;
                        sp->server_seq_no = 22;
                        sp->client_work = ST_ServerDir2ClientNoGet(bw, sp, side);
                        ret = 1;
                ...
                if (sp->fcc.wish_count[side])
                {
                    if (--sp->fcc.wish_count[side] == 0)
                    {
                        if (sp->battlemon[side].hp)
                        {
                            ...
                            LoadBattleSubSeqScript(sp, ARC_BATTLE_SUB_SEQ, SUB_SEQ_HANDLE_WISH_CAME_TRUE);
                            sp->next_server_seq_no = sp->server_seq_no;
                            sp->server_seq_no = 22;
                            ret = 1;
                        }
        ...
        case FCC_RAIN:
            if (sp->field_condition & WEATHER_RAIN_ANY)
            {
                ...
                if (sp->field_condition & WEATHER_RAIN_PERMANENT)
                {
                    ...
                    LoadBattleSubSeqScript(sp, ARC_BATTLE_SUB_SEQ, SUB_SEQ_CONTINUE_WEATHER);
                    sp->next_server_seq_no = sp->server_seq_no;
                    sp->server_seq_no = 22;
                }
                else
#endif // DISABLE_END_OF_TURN_WEATHER_MESSAGE
                {
                    ...
                    {
                        LoadBattleSubSeqScript(sp, ARC_BATTLE_SUB_SEQ, SUB_SEQ_RAIN_ENDS);
                        sp->next_server_seq_no = sp->server_seq_no;
                        sp->server_seq_no = 22;
                    }
#ifndef DISABLE_END_OF_TURN_WEATHER_MESSAGE
        ...
        case FCC_SANDSTORM:
            if (sp->field_condition & WEATHER_SANDSTORM_ANY)
            {
                ...
                else
                {
                    if(--sp->fcc.weather_count == 0)
                    {
                        LoadBattleSubSeqScript(sp, ARC_BATTLE_SUB_SEQ, SUB_SEQ_SANDSTORM_ENDS);
                        sp->next_server_seq_no = sp->server_seq_no;
                        sp->server_seq_no = 22;
                    }
                    else
        ...
        case FCC_SUNNY:
            if (sp->field_condition & WEATHER_SUNNY_ANY)
            {
...
#endif // DISABLE_END_OF_TURN_WEATHER_MESSAGE
                {
                    if (--sp->fcc.weather_count == 0)
                    {
                        LoadBattleSubSeqScript(sp, ARC_BATTLE_SUB_SEQ, SUB_SEQ_SUNLIGHT_FADES);
                        sp->next_server_seq_no = sp->server_seq_no;
                        sp->server_seq_no = 22;
                    }
#ifndef DISABLE_END_OF_TURN_WEATHER_MESSAGE
        ...
        case FCC_HAIL:
            if (sp->field_condition & WEATHER_HAIL_ANY)
            {
                ...
                else
                {
                    if (--sp->fcc.weather_count == 0)
                    {
                        LoadBattleSubSeqScript(sp, ARC_BATTLE_SUB_SEQ, SUB_SEQ_HAIL_STOPPED);
                        sp->next_server_seq_no = sp->server_seq_no;
                        sp->server_seq_no = 22;
                    }
                    else
        ...
        case FCC_GRAVITY:
            if (sp->field_condition & FIELD_STATUS_GRAVITY)
            {
                ...
                {
                    LoadBattleSubSeqScript(sp, ARC_BATTLE_SUB_SEQ, SUB_SEQ_GRAVITY_NORMAL);
                    sp->next_server_seq_no = sp->server_seq_no;
                    sp->server_seq_no = 22;
                    ret = 1;
                }
