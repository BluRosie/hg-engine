...
static const HiddenItemData *HiddenItemFindByIndex(u16 index)
{
    u32 i;

    for (i = 0; i < NELEMS(sHiddenItemParam); i++) {
        if (sHiddenItemParam[i].index == index) {
            return &sHiddenItemParam[i];
        }
    }
...
{
    const HiddenItemData *hiddenItem = HiddenItemFindByIndex(HiddenItemScriptNoToHiddenItemIdx(script));
