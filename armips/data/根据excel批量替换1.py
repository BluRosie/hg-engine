...
def main():
    ...
    excel_file_path = '基础数据表.xlsx'
    mondata_file_path = 'mondata.s'
    mondata_out_file_path = 'mondata.s'

    if os.path.exists(mondata_file_path):
        try:
            log_message("正在创建备份文件...")
            if os.path.exists('mondata_bak.s'):
                os.remove('mondata_bak.s')
                log_message("删除已存在的备份文件")
            os.rename(mondata_file_path, 'mondata_bak.s')
            log_message("成功创建备份文件: mondata_bak.s")
        except Exception as e:
            log_message(f"创建备份文件时发生错误: {e}")
            exit()

    mon_data_dict = read_excel_to_dict(excel_file_path, 'Poke表')
    replace_stats_and_abilities_in_c_file('mondata_bak.s', mondata_out_file_path, mon_data_dict)
