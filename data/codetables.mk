...
LEVELUPLEARNSETOFFSET_TARGET := $(BUILD)/a033/0_1
LEVELUPLEARNSETOFFSET_DEPENDENCIES := data/LevelupLearnsetOffsets.c
LEVELUPLEARNSETOFFSET_OBJS := $(patsubst data/%.c,build/%.o,$(LEVELUPLEARNSETOFFSET_DEPENDENCIES))
LEVELUPLEARNSETOFFSET_BIN := $(patsubst data/%.c,build/%.bin,$(LEVELUPLEARNSETOFFSET_DEPENDENCIES))

$(LEVELUPLEARNSET_DEPENDENCIES) $(LEVELUPLEARNSETOFFSET_DEPENDENCIES): $(LEARNSETS_DATA)
	@echo "levelup moves:"
	$(PYTHON) scripts/build_learnsets.py --learnsets $(LEARNSETS_DATA) --levelupout ${LEVELUPLEARNSET_DEPENDENCIES} --levelupoffsetout ${LEVELUPLEARNSETOFFSET_DEPENDENCIES}

$(LEVELUPLEARNSET_BIN): $(LEVELUPLEARNSET_DEPENDENCIES)
	$(CC) $(CFLAGS) -c $< -o $(LEVELUPLEARNSET_OBJS)
	$(OBJCOPY) -O binary $(LEVELUPLEARNSET_OBJS) $@

$(LEVELUPLEARNSETOFFSET_BIN): $(LEVELUPLEARNSETOFFSET_DEPENDENCIES)
	$(CC) $(CFLAGS) -c $< -o $(LEVELUPLEARNSETOFFSET_OBJS)
	$(OBJCOPY) -O binary $(LEVELUPLEARNSETOFFSET_OBJS) $@

NARC_FILES += $(LEVELUPLEARNSET_BIN) $(LEVELUPLEARNSETOFFSET_BIN)
REQUIRED_DIRECTORIES += $(BUILD)/a033


EGGLEARNSET_TARGET := $(BUILD)/kowaza/0_0
...
EGGLEARNSETOFFSET_TARGET := $(BUILD)/kowaza/0_1
EGGLEARNSETOFFSET_DEPENDENCIES := data/EggLearnsetOffsets.c
EGGLEARNSETOFFSET_OBJS := $(patsubst data/%.c,build/%.o,$(EGGLEARNSETOFFSET_DEPENDENCIES))
EGGLEARNSETOFFSET_BIN := $(patsubst data/%.c,build/%.bin,$(EGGLEARNSETOFFSET_DEPENDENCIES))

$(EGGLEARNSET_DEPENDENCIES) $(EGGLEARNSETOFFSET_DEPENDENCIES): $(LEARNSETS_DATA)
	@echo "Generating egg move tables..."
	$(PYTHON) scripts/build_learnsets.py --learnsets $(LEARNSETS_DATA) --eggout $(EGGLEARNSET_DEPENDENCIES) --eggoffsetout $(EGGLEARNSETOFFSET_DEPENDENCIES)

$(EGGLEARNSET_BIN): $(EGGLEARNSET_DEPENDENCIES)
	$(CC) $(CFLAGS) -c $< -o $(EGGLEARNSET_OBJS)
	$(OBJCOPY) -O binary $(EGGLEARNSET_OBJS) $@

$(EGGLEARNSETOFFSET_BIN): $(EGGLEARNSETOFFSET_DEPENDENCIES)
	$(CC) $(CFLAGS) -c $< -o $(EGGLEARNSETOFFSET_OBJS)
	$(OBJCOPY) -O binary $(EGGLEARNSETOFFSET_OBJS) $@

NARC_FILES += $(EGGLEARNSET_BIN) $(EGGLEARNSETOFFSET_BIN)
REQUIRED_DIRECTORIES += $(BUILD)/kowaza
