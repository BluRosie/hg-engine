...
def decompress():
    ...
    with open("base/overarm9.bin", 'rb+') as rom:
        rom.seek((1*0x20)+0x1F) #write 0 to (num*0x20)+0x1F to make game consider overlay num decompressed (and call decompress below)
        bunh = bytes([0x0])
        rom.write(bytes(bunh))
        rom.seek((2*0x20)+0x1F)
        rom.write(bytes(bunh))
        rom.seek((6*0x20)+0x1F)
        rom.write(bytes(bunh))
        rom.seek((7*0x20)+0x1F)
        rom.write(bytes(bunh))
        rom.seek((10*0x20)+0x1F)
        rom.write(bytes(bunh))
        rom.seek((12*0x20)+0x1F)
        rom.write(bytes(bunh))
        rom.seek((14*0x20)+0x1F)
        rom.write(bytes(bunh))
        rom.seek((15*0x20)+0x1F)
        rom.write(bytes(bunh))
        rom.seek((18*0x20)+0x1F)
        rom.write(bytes(bunh))
        rom.seek((63*0x20)+0x1F)
        rom.write(bytes(bunh))
        rom.seek((96*0x20)+0x1F)
        rom.write(bytes(bunh))
        rom.close()
