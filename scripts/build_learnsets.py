...
def write_learnset_constants_header(max_num_machine_moves, max_num_levelup_moves, max_num_egg_moves, output_path):
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    ...
    with open(output_path, "w") as f:
        ...
        f.write(f"#define MAX_MACHINE_MOVES {max_num_machine_moves}\n")
        f.write(f"#define MAX_LEVELUP_MOVES {max_num_levelup_moves}\n")
        f.write(f"#define MAX_EGG_MOVES     {max_num_egg_moves}\n\n")
        f.write(f"#define MACHINE_LEARNSETS_BITFIELD_COUNT (MAX_MACHINE_MOVES / 32)\n\n")
        f.write("#endif // GENERATED_LEARNSET_CONSTANTS_H\n")


def write_machine_data(species_dict, moves_dict, species_learnsets, machine_moves, max_num_machine_moves, output_path):
    move_to_index = {
        move_name: idx
    ...
    with open(output_path, "w") as out:
        ...
        for species_id in range(max_species_index + 1):
            ...
            parts = [0] * (max_num_machine_moves // 32)
            for move in learnset:
                move_index = move_to_index.get(move)
                if move_index is not None and move_index < max_num_machine_moves:
                    word = move_index // 32
                    bit = move_index % 32
                    parts[word] |= (1 << bit)
...
if __name__ == "__main__":
    ...
    max_num_machine_moves = ((len(machine_moves) + 31) // 32) * 32  # ensure divisible by 32

    if args.constsout:
        write_learnset_constants_header(max_num_machine_moves, max_num_levelup_moves, max_num_egg_moves, "include/constants/generated/learnsets.h")
        write_learnset_constants_inc(max_num_levelup_moves, "armips/include/generated/levelup.s")

    if args.machineout:
        write_machine_data(species_dict, moves_dict, species_learnsets, machine_moves, max_num_machine_moves, args.machineout)
